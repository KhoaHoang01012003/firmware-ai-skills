---
name: firmware-attack-surface-graph
description: Use when service inventory exists and target selection needs evidence-backed attack surface structure.
---
...
Build a service-to-input-vector-to-parser-to-sink attack-surface graph. This skill is part of the discovery-first firmware research suite for authorized binary/service exploitability assessment. It preserves blocker-first workflow, compact local evidence, and provenance-first claims.

## Routing

```yaml
role: domain
purpose: Build a service-to-input-vector-to-parser-to-sink attack-surface graph.
when_to_use:
  - Use when service inventory exists and target selection needs evidence-backed attack surface structure.
when_not_to_use:
...
completion_gate:
  - attack_surface_graph.json is written with schema_version, provenance, warnings, errors, evidence labels, and blockers for missing inputs.
reentry_triggers:
...
1. Create graph nodes for services, inputs, parsers, sinks, privileges, and auth boundaries.
2. Create edges only when local artifacts support them.
3. Rank targets by reachability, parser complexity, privilege, and evidence gaps.
4. Write blockers for missing graph evidence.
