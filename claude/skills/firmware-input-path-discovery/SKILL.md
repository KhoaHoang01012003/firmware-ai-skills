---
name: firmware-input-path-discovery
description: Use when a selected service or sink candidate needs source/input controllability analysis.
---
...
Discover attacker-controllable input vectors and path chains into selected services. This skill is part of the discovery-first firmware research suite for authorized binary/service exploitability assessment. It preserves blocker-first workflow, compact local evidence, and provenance-first claims.

## Routing

```yaml
role: domain
purpose: Discover attacker-controllable input vectors and path chains into selected services.
when_to_use:
  - Use when a selected service or sink candidate needs source/input controllability analysis.
when_not_to_use:
...
required_inputs:
  ...
  - selected binary or service
produces:
  - input_vectors.json
  - input_path_chains.json
completion_gate:
  - input_vectors.json is written with schema_version, provenance, warnings, errors, evidence labels, and blockers for missing inputs.
reentry_triggers:
...
1. Inspect CLI, network, IPC, file, environment, and update/config inputs.
2. Trace candidate path chains from source to parser.
3. Use CodeQL, Semgrep, Joern, strings, xrefs, and binary-map evidence as static leads only.
4. Set behavior_claim_allowed=false for static-only path claims.
