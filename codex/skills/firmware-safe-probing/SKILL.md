---
name: firmware-safe-probing
description: Use when hypothesis, input vector, expected observation, negative control, stop condition, and safety boundary are defined.
---

# Firmware Safe Probing

## Overview

Generate and run controlled probes with negative controls, stop conditions, and authorization boundaries. This skill is part of the discovery-first firmware research suite for authorized binary/service exploitability assessment. It preserves blocker-first workflow, compact local evidence, and provenance-first claims.

## Routing

```yaml
role: interaction
purpose: Generate and run controlled probes with negative controls, stop conditions, and authorization boundaries.
when_to_use:
  - Use when hypothesis, input vector, expected observation, negative control, stop condition, and safety boundary are defined.
when_not_to_use:
...
completion_gate:
  - probe_plan.json is written with schema_version, provenance, warnings, errors, evidence labels, and blockers for missing inputs.
reentry_triggers:
...
1. Declare sandbox or live mode and no network by default for Python sandbox work.
2. Keep destructive actions disabled by default; destructive actions require explicit user authorization.
3. Run negative controls before interpreting effects.
4. Stop immediately on safety boundary, unexpected broad effect, or missing local evidence.
