---
name: firmware-intake-normalization
description: Use when a raw firmware artifact or extraction directory needs normalized identity and provenance before analysis.
---
...
Normalize firmware identity, hashes, extraction metadata, and initial binary/service seeds. This skill is part of the discovery-first firmware research suite for authorized binary/service exploitability assessment. It preserves blocker-first workflow, compact local evidence, and provenance-first claims.

## Routing

```yaml
role: domain
purpose: Normalize firmware identity, hashes, extraction metadata, and initial binary/service seeds.
when_to_use:
  - Use when a raw firmware artifact or extraction directory needs normalized identity and provenance before analysis.
when_not_to_use:
...
produces:
  ...
  - binary seed list
completion_gate:
  - firmware_manifest.json is written with schema_version, provenance, warnings, errors, evidence labels, and blockers for missing inputs.
reentry_triggers:
...
1. Hash firmware files.
2. Record extraction tools and blockers.
3. Identify rootfs candidates and initial binary/service seeds.
4. Write firmware_manifest.json with provenance.
