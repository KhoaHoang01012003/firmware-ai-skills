---
name: firmware-device-context-research
description: Use when firmware identity needs official model/version context or advisory context before duplicate checking.
---

# Firmware Device Context Research

## Overview

Research vendor, model, version, hardware, and public context without turning public claims into runtime truth. This skill is part of the discovery-first firmware research suite for authorized binary/service exploitability assessment. It preserves blocker-first workflow, compact local evidence, and provenance-first claims.

## Routing

```yaml
role: domain
purpose: Research vendor, model, version, hardware, and public context without turning public claims into runtime truth.
when_to_use:
  - Use when firmware identity needs official model/version context or advisory context before duplicate checking.
when_not_to_use:
...
produces:
  - model_research.json
  - context_blockers.json
completion_gate:
  - model_research.json is written with schema_version, provenance, warnings, errors, evidence labels, and blockers for missing inputs.
reentry_triggers:
...
1. Collect official vendor and version references.
2. Record contradictions and dates.
3. Separate public claims from local evidence.
4. Defer duplicate CVE decisions to firmware-duplicate-cve-research.
