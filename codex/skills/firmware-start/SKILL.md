...
4. Apply default safety boundaries: local artifact analysis, no public targeting, no mass scanning, no destructive probing, no upload of raw firmware or evidence, no stored credentials, and no invented CVE IDs.
5. If authorization is ambiguous, write blockers.json or ask for confirmation before runtime interaction, probing, login, or exploitability claims.
6. Write next_skill_decision.json that selects firmware-discovery-orchestrator as the handoff skill.
...
When `requested_goal.emulation_success_gate=true`, starter_request.json should preserve that goal so downstream skills must produce `runtime_readiness.json`, `reachable_services.json`, `ui_access_status.json`, and `emulation_success.json` or an explicit blocker.
